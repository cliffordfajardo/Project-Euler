...
2. Since we want to find out the total of a number, at the end, it makes to declare a variable, total, which we will
   use to keep adding the multiples of 3 or 5, and make the inital value 0. I tried to leave the initial value of total undefined like this (var total;) , but undefined + a number equals NaN, so we need 0 to start.
...
1.  A new use of the decrement operator with a while loop:
    ...
    0 has a very special meaning in javascript, it can mean false also. So when my number is decremented all the way to down 0 it tells the while loop, "the condition is no longer true". Initially, I thought my (num--) would just decrement down infintely and cause an infinete loop producing negative numbers, but that's not the case.
